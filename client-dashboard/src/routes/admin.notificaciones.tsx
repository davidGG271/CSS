...
import { getClientes, getProducts, sendCampaign } from "@/lib/admin-api";
...
function NotificationsPage() {
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      const compressedProducts = await Promise.all(targetProducts.map(async (p) => {
        let compressedImage = p.image || "";
        if (compressedImage && compressedImage.length > 50000 && compressedImage.startsWith("data:image")) {
           try {
             compressedImage = await compressImage(compressedImage, 300, 0.5); 
           } catch(e) {
    ...
    onError: (e: any) => {
      console.error(e);
      const errorMessage = e.response?.data?.message || e.response?.statusText || e.message || "Error al enviar la campaña";
      toast.error(errorMessage === "Payload Too Large" ? "Error 413: Las imágenes son demasiado pesadas. Por favor, sube el backend con el nuevo límite." : errorMessage);
    }
  });
